...
impl BVH {
    pub fn new(mut hittable:Vec<Box<dyn Hittable>>, time0: f32, time1: f32) -> Self {
        ...
        fn axis_range(hittable: &Vec<Box<dyn Hittable>>, time0: f32, time1: f32, axis: usize) -> f32 {
            let (min, max) = hittable.iter().fold((f32::MAX, f32::MIN), |(bmin, bmax),hit| {
        ...
        let mut axis_ranges: Vec<(usize, f32)> = (0..3)
            .map(|a| (a, axis_range(&hittable, time0, time1, axis)))
            .collect();
        ...
        match len {
            ...
            _ => {
                ...
                let bbox = surrounding_box(&left.bb, &right.bbox);
